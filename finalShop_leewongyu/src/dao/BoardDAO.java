...
public class BoardDAO {
	...
	/** print board list in current page */
	...
	private int getNextPage(int curPage, int totalPage) {
		if (curPage != totalPage) return curPage + 1;
		Util.showErrorMsg("다음 페이지가 없습니다");
		return curPage;
	}

	/** print selected board contents */
	...
	private boolean deleteValidBoard() {
		...
		for (int i = 0; i < boardList.size(); i++)
			if (isValidDelete(boardList.get(i), num)) {
				boardList.remove(i);
				return true;
			}
		printDeleteError();
	...
	/** delete value is exist? */
	private boolean isValidDelete(Board board, int num) {
		if (board.getBoardNum() == num
				&& (cont.getNext().equals("AdminBoard") || board.getId().equals(cont.getLoginId())))
			return true;
		return false;
	}
	...
	/** print board list in setting page */
	...
	public void addBoard() {
		...
		boardList.add(
				new Board(Board.getNum() + "", title, contents, cont.getLoginId(), LocalDate.now().toString(), 0 + ""));
		Board.plusNum();
		Util.showMsg("게시글 추가 완료");
	}

	/** delete a board for login member */
	...
	public String getSaveData() {
		...
		String data = "";
		for (Board b : boardList)
			data += String.format("%d/%s/%s/%s/%s/%d\n", b.getBoardNum(), b.getTitle(), b.getContents(), b.getId(),
					b.getDate(), b.getHits());
